    ...
    class Currency
    {
        ...
        public Currency()
        {
            ...
            try
            {
                ...
                var stream = http.GetStreamAsync(sourceUrl).Result;
                xml = XmlReader.Create(stream); //tries to download XML file and create the Reader object
            }
                ...
                while (xml.Read())
                {
                    if (xml.Name == "Cube")
                    {
                        if (xml.AttributeCount == 1)
                        {
                            ...
                            this._date = DateTime.Parse(xml.Value); // gets the date on which this rates are valid
                        }
                        if (xml.AttributeCount == 2)
                        {
                            xml.MoveToAttribute("currency");
                            var currency = xml.Value;
                            ...
                            this.exchangeRates.Add(currency, rate); //adds currency and rate to exchange rate table
                        }
